...
public class MapGenerator : MonoBehaviour
{
    ...
    public IReadOnlyList<Vector2Int> EnemySpawnHolePositions {
        get {
            return _enemySpawnHolePositions;
        }
    }
    ...
    {
        if (_holeRadiusValues.TryGetValue(holePosition, out (float homeRadius, float territoryRadius) radiusValues)) {
            return radiusValues;
        }
        return (0f, 0f);
    }
    ...
    public IEnumerator GenerateMapAsync(IInitializationProgress progress = null)
    {
        ...
        if (useProceduralTerrain) {
            ...
            GenerateNoiseMapJob noiseJob = new GenerateNoiseMapJob {
        ...
        if (useGradientMap && gradientMode == GradientMode.Procedural) {
            GenerateGradientMapJob gradientJob = new GenerateGradientMapJob {
        ...
        CalculateTerrainTilesJob tileJob = new CalculateTerrainTilesJob {
    ...
    private void FillDisconnectedAreas()
    {
        ...
        while (queue.Count > 0) {
            ...
            Vector2Int[] neighbors = {
                new Vector2Int(x + 1, y), new Vector2Int(x - 1, y), new Vector2Int(x, y + 1), new Vector2Int(x, y - 1)
            };
    ...
    private TileBase GetTileForPosition(int x, int y)
    {
        ...
        if (useProceduralTerrain && _noiseMap != null) {
            ...
            if (noiseValue >= lowWallThreshold) {
